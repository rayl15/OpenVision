...
public final class KokoroTTS {
  ...
  public func generateAudio(voice: MLXArray, language: Language, text: String, speed: Float = 1.0) throws -> ([Float], [MToken]?) {
    ...
    MLX.eval(durationFeatures) // Checkpoint 1
    ...
    MLX.eval(asrFeatures) // Checkpoint 5
    ...
    // FORCE EVALUATION: Critical for iOS Memory Management
    let outputAudio = audio[0].asArray(Float.self)
    
    // Explicitly clear intermediates from the graph
    MLX.eval(audio)
    
    // Clear Metal cache to free unused buffers
    GPU.clearCache()
    
    return (outputAudio, tokenArray)
  }
  ...
  private func createAlignmentTarget(durations: MLXArray, batchSize: Int) -> MLXArray {
    ...
    let indices = MLX.concatenated(
      durations.enumerated().map { index, duration in
        ...
        let repeated = MLX.repeated(MLXArray([index]), count: frameCount)
        MLX.eval(repeated) // evaluating inside loop keeps graph small
        return repeated
      }
    )
